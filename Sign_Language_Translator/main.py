...
class ShowVideo(QtCore.QObject):
    ...
    def Api_trans(self):  # 이미지 추론 및 번역 함수
        ...
        client_id = "이 부분을 채워주세요"  # Papago 개발자센터에서 발급받은 Client ID 값
        client_secret = "이 부분을 채워주세요"  # Papago 개발자센터에서 발급받은 Client Secret 값
        ...
        # Google Automl vision에서 학습시킨 프로젝트, 모델명, 임계값을 설정.
        project_id = "이 부분을 채워주세요"
        compute_region = "이 부분을 채워주세요"
        model_id = "이 부분을 채워주세요"
        score_threshold = "이 부분을 채워주세요"
        ...
        # label명에 특수문자를 사용하는 것이 불가능하기 때문에
        # 미리 설정한 label 명을 특수문자 등으로 변경해주면 된다.
...
if __name__ == '__main__':
    ...
    horizontal_layout2.addWidget(push_button1)
    ...
    push_button1.setGeometry(QtCore.QRect(690, 110, 250, 150))
    ...
    push_button1.setIconSize(QtCore.QSize(250, 150))
    push_button1.setObjectName("pushButton")
    push_button1.setStyleSheet("border:0px solid #ffffff")

    push_button2.setGeometry(QtCore.QRect(690, 390, 250, 150))
    push_button2.setText("")
    icon2 = QtGui.QIcon()
    icon2.addPixmap(QtGui.QPixmap("icon/question.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
    push_button2.setIcon(icon2)
    push_button2.setIconSize(QtCore.QSize(250, 150))
    ...
    spinBox.setGeometry(QtCore.QRect(860, 900, 91, 41))
    ...
    main_window.setWindowTitle(_translate("Form", " Sign Language Translator"))
    main_window.show()
    vid.startVideo()
